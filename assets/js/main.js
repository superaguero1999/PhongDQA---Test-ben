(function initForm15Main(global) {
  ...
  /** Đang chạy Refresh (nút hoặc tự động) — dùng để khóa nhập liệu trong lúc tải dữ liệu. */
  ...
  function applyManualLock() {
    /** Khóa khi chưa refresh phiên, hoặc đang chạy Refresh (thủ công / tự động). */
    const disabled = !hasRefreshedThisSession || isRefreshing;
  ...
  function setEditingStatusIfReady() {
    if (!hasRefreshedThisSession || isRefreshing) return;
